...
namespace Kenty
{
    ...
    /// 3D キャラクター付きの Spatial Anchor を設置・保存・読み込みするコンポーネント。
    /// シーン内に存在するアンカーは常に 1 つのみ。
    /// </summary>
    public class CharacterAnchorPlacer : MonoBehaviour
    {
        private const string AnchorUuidKey = "CharacterAnchorUuid";
        ...
        [Header("参照")]
        [SerializeField]
        [Tooltip("GlobalMeshScanner への参照（Load 時にメッシュを非表示にする）")]
        private GlobalMeshScanner _meshScanner;
        ...
        public async void SaveAnchor()
        {
            ...
            if (!result.Success)
            {
                ...
                Debug.LogWarning("[CharacterAnchorPlacer] アンカーの保存に失敗しました。");
                return;
            }
            ...
            Debug.Log($"[CharacterAnchorPlacer] アンカーを保存しました。UUID: {uuid}");
        }
        ...
        public async void LoadSavedAnchor()
        {
            ...
            if (!loadResult.Success || unboundAnchors.Count == 0)
            {
                ...
                Debug.LogWarning("[CharacterAnchorPlacer] アンカーの読み込みに失敗しました。");
                return;
            }
            ...
            if (!unboundAnchor.Localized)
            {
                ...
                if (!localized)
                {
                    ...
                    Debug.LogWarning("[CharacterAnchorPlacer] アンカーのローカライズに失敗しました。");
                    return;
                }
            }
            ...
            // GlobalMesh の描画を非表示にする
            if (_meshScanner is not null)
            {
                _meshScanner.SetMeshVisible(false);
            }

            SetStatus("Anchor Loaded");
            Debug.Log($"[CharacterAnchorPlacer] アンカーを復元しました。UUID: {uuid}");
        }

        /// <summary>
        /// 配置モード中の毎フレーム処理。
        /// Ray を飛ばして可視化し、ボタン押下でアンカーを設置する。
        /// </summary>
        ...
        private async void PlaceAnchor(Vector3 position)
        {
            ...
            Debug.Log($"[CharacterAnchorPlacer] アンカーを設置しました。位置: {position}");
        }
        ...
        private GameObject CreateAnchorVisual(Vector3 position, Quaternion rotation)
        {
            ...
            anchorObject.name = "CharacterAnchor";
            return anchorObject;
        }
        ...
        public void AnimationNext()
        {
            ...
            {
                // _currentAnimator.SetBool("Next", true);
        ...
        public void AnimationBack()
        {
            if (_currentAnimator is not null)
            {
                // _currentAnimator.SetBool("Back", true);
